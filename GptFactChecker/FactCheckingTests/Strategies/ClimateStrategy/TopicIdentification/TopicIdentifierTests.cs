...
""";
    ...
    public async Task Should_Return_Claims_When_Correct_ArgumentDataList_And_CompatibleFacts_Are_Passed()
    {
        ...
        _mockGptResponseParser.Setup(parser => parser.ParseGptResponseFunctionCall<ReferenceMatchingResponse>(
                It.IsAny<string>(), It.IsAny<string>()))
            .Returns(new ReferenceMatchingResponse { ClaimsWithReferences = expectedClaims });
    ...
    public async Task Should_Return_Claims_And_Combine_References_When_Same_ClaimId_Exists()
    {
        ...
        _mockGptResponseParser.SetupSequence(parser => parser.ParseGptResponseFunctionCall<ReferenceMatchingResponse>(
                It.IsAny<string>(), It.IsAny<string>()))
            .Returns(new ReferenceMatchingResponse { ClaimsWithReferences = new List<ClaimWithReferences> { new ClaimWithReferences { ClaimId = "1", ReferenceIds = new List<string> { "Ref1", "Ref2" } } } })
            .Returns(new ReferenceMatchingResponse { ClaimsWithReferences = new List<ClaimWithReferences> { new ClaimWithReferences { ClaimId = "1", ReferenceIds = new List<string> { "Ref3", "Ref4" } } } })
            .Returns(new ReferenceMatchingResponse { ClaimsWithReferences = new List<ClaimWithReferences> { new ClaimWithReferences { ClaimId = "1", ReferenceIds = new List<string> { "Ref5", "Ref6" } } } });
